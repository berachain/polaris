...
var _ = Describe("Network", func() {
	...
	It("should support eth_getBalance", func() {
		...
		balance, err := client.BalanceAt(ctx, tf.Address("0"), nil)
		Expect(err).ToNot(HaveOccurred())
	...
	It("should deploy, mint tokens and check balance, eth_getTransactionByHash", func() {
		// Deploy the contract
		erc20Contract := DeployERC20(tf.GenerateTransactOpts("0"), client)

		// Mint tokens
		tx, err := erc20Contract.Mint(tf.GenerateTransactOpts("0"),
			tf.Address("0"), big.NewInt(100000000))
		Expect(err).ToNot(HaveOccurred())
		...
		erc20Balance, err := erc20Contract.BalanceOf(&bind.CallOpts{}, tf.Address("0"))
		Expect(err).ToNot(HaveOccurred())
