...
var _ = Describe("Staking Precompile", func() {
	When("Calling the precompile directly", func() {
		It("should get the validators", func() {
			validators, err := stakingPrecompile.GetActiveValidators(nil)
			Expect(err).ToNot(HaveOccurred())
			Expect(validators).To(ContainElement(validator))
		})
		It("should get the delegations", func() {
	...
	When("Calling the precompile from a contract", func() {
		var (
			wrapperContract *tbindings.LiquidStaking
		)
		BeforeEach(func() {
		...
		It("should get total delegated", func() {
			value, err := wrapperContract.TotalDelegated(nil)
			Expect(err).ToNot(HaveOccurred())
			Expect(value.Cmp(big.NewInt(0))).To(Equal(0))
		})
		It("should get active validators", func() {
			addresses, err := wrapperContract.GetActiveValidators(nil)
			Expect(err).ToNot(HaveOccurred())
			Expect(addresses).To(HaveLen(1))
			Expect(addresses[0]).To(Equal(validator))
		})
		It("should be able to delegate", func() {
			// Send tokens to the contract
			txr := tf.GenerateTransactOpts("")
			txr.GasLimit = 0
			txr.Value = big.NewInt(100000000000)
			tx, err := wrapperContract.Delegate(txr, big.NewInt(100000000000))
			Expect(err).ToNot(HaveOccurred())
			ExpectMined(tf.EthClient, tx)
			ExpectSuccessReceipt(tf.EthClient, tx)
