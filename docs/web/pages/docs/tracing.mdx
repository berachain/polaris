...
There are two different types of transactions in the Polaris EVM: value transfers and contract executions. A value transfer simply moves
...
tracing solves. Polaris' interface traces transactions by re-running them locally and collecting data about precisely what was executed by the EVM.

## State Availability

In its simplest form, tracing a transaction entails requesting the Polaris node to reexecute the desired transaction with 
varying degrees of data collection and have it return an aggregated summary. In order for a Polaris node to reexecute a transaction, 
all historical state accessed by the transaction must be available. 

By Default, all Polaris nodes are archval nodes.
It is possible to run a Polaris light client but for EVM tracing, a full node is required.
...
Polaris has the ability to produce basic transaction traces in the form of raw EVM opcode traces. These traces contain detailed information about every EVM instruction executed during a transaction, 
...
The tracing API of Polaris allows the user to specify a tracer parameter, 
...
resource-intensive to process. To address these issues, Polaris includes 
...
Besides the built-in tracers, Polaris also allows users to create custom tracers 
