...
There are two different types of transactions in Stargazer EVM: value transfers and contract executions. A value transfer just moves
the base token from one account to another. A contract interaction executes some code stored at a contract address which can include altering 
...
tracing solves. Stargazer's Geth interface traces transactions by re-running them locally and collecting data about precisely what was executed by the EVM.

## State Availability

In its simplest form, tracing a transaction entails requesting the Stargazer node to reexecute the desired transaction with 
varying degrees of data collection and have it return an aggregated summary. In order for a Stargazer node to reexecute a transaction, 
all historical state accessed by the transaction must be available. 

By Default, all Stargazer nodes are archval nodes.
It is possible to run a Stargazer light client but for EVM tracing, a full node is required.
...
Stargazer has the ability to produce basic transaction traces in the form of raw EVM opcode traces. These traces contain detailed information about every EVM instruction executed during a transaction, 
...
The tracing API of Stargazer allows the user to specify a tracer parameter, 
...
resource-intensive to process. To address these issues, Stargazer includes 
...
Besides the built-in tracers, Stargazer also allows users to create custom tracers 
