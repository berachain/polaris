...
In order to point this dapp towards a Polaris local node, 
...
We will need to add some utility functions to our dApp. These functions will help us interact with the Polaris precompile.
...
function Page() {
  return (
    <Flex justifyContent={'center'} alignItems={'center'} height={'100vh'} direction={'column'}>
      ...
      <Text fontSize={'xl'}>Ensure you have a Local <Link color={'#1da1f1'} href='TODO ADD LINK'>Polaris Node running.</Link></Text>
      <Staking />
      <Text fontSize={'xs'}>To learn more about Polaris please reference our <Link color={'#1da1f1'} href='TODO ADD LINK'>docs</Link></Text>
    </Flex>
  )
}
...
Prior to Polaris' precompiled contracts, the UX of this task would be more complicated and foreign as it involves
