...
function Page() {
  return (
    <Flex justifyContent={'center'} alignItems={'center'} height={'100vh'} direction={'column'}>
      ...
      <Text fontSize={'xl'}>Ensure you have a Local <Link color={'#1da1f1'} href='TODO ADD LINK'>Stargazer Node running.</Link></Text>
      <Staking />
      <Text fontSize={'xs'}>To learn more about Stargazer please reference our <Link color={'#1da1f1'} href='TODO ADD LINK'>docs</Link></Text>
    </Flex>
  )
}
...
Prior to Stargazer's precompiled contracts, the UX of this task would be more complicated and foreign as it involves
...
You can view this example and others like it on our examples repository on Github [here](https://github.com/berachain/stargazer-examples)