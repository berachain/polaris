...
- Basic understanding of the [Stargazer's architecture](../../architecture) 
...
- You have [installed
  ...
  walk-through.
- You have a running instance of Stargazer EVM. See [Local Machine](../../running-a-node/running-locally) to run Stargazer locally.
...
<Callout emoji="💡">
  Ensure that you have a local instance of Stargazer Running. See [Local Machine](../../running-a-node/running-locally) to run Stargazer locally.
</Callout>
...
```bash
FOUNDRY_PROFILE=stargazer_local forge create Counter --private-key=xxxxxxxxxxx
```
...
- It takes two arguments in its constructor: the address of [Staking Module Precompile Contract Address](../../precompile-overview), and the address of the validator that the contract will delegate to.
...
<Callout emoji="💡">
The default staking precompile contract address is `0x0000000000000000000000000000000000000999`. More information
about precompiles can be found [here](../../precompile-overview)
</Callout>

```bash
FOUNDRY_PROFILE=stargazer_local forge create src/LiquidStaking.sol:LiquidStaking --private-key=xxxxxxxxxxx --constructor-args  “<name>“ “<symbol>” “<staking_precompiled_contract_address>” “<validator_address>”
```
...
```bash
FOUNDRY_PROFILE=stargazer_local cast send --private-key=xxxxxxxxxxx <deployed_contract_address> ‘delegate()’ 1
```

#### 9. 🎉 Congratulations! 🎉
Congratulations! You have successfully created and deployed a Liquid Staking contract on Stargazer via Foundry!